...
const suiteApps = [
  ...
  { name: `ZiggyReviews`, desc: `Reviews & Reputation`, color: `#f59e0b`, domain: `ziggyreviews.com`, current: true },
    ...
    <div className="bg-[#0a0a0a] min-h-screen" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
      ...
      <section className="pt-20 pb-16 px-4 text-center">
        <div className="max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#f59e0b]/10 border border-[#f59e0b]/20 text-[#f59e0b] text-xs font-medium mb-6">ZiggyTech Business Suite</div>
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-6 leading-tight">10 tools. One workspace.<br /><span style={{ color: '#f59e0b' }}>One price.</span></h1>
          <p className="text-xl text-[#b3b3b3] max-w-2xl mx-auto mb-8">ZiggyReviews is part of the ZiggyTech Business Suite — 10 tightly integrated tools to run your entire business.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="https://ziggybusiness.com" className="px-6 py-3 bg-[#f59e0b] text-white rounded-lg font-medium hover:opacity-90 transition-colors">Get all 10 apps — $179/mo</a>
      ...
      <section className="py-16 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {suiteApps.map((a) => (
              <div key={a.name} className={`bg-[#111111] border rounded-xl p-5 ${a.current ? 'border-[#f59e0b]/40' : 'border-[#1f1f1f]'}`}>
                ...
                {a.current && <span className="inline-block mt-2 px-2 py-0.5 bg-[#f59e0b]/10 text-[#f59e0b] text-xs rounded-full">Current app</span>}
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {automations.map((a, i) => (
              <div key={i} className="bg-[#111111] border border-[#1f1f1f] rounded-xl p-4 flex items-center gap-4">
                ...
                <svg className="w-5 h-5 text-[#f59e0b] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                <div className="flex-1"><p className="text-[#b3b3b3] text-sm">Action: <span className="text-[#f59e0b]">{a.action}</span></p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-4 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <a href="https://ziggybusiness.com" className="inline-flex items-center gap-2 px-10 py-5 bg-[#f59e0b] text-white rounded-xl font-bold text-xl hover:opacity-90 transition-all">Explore ZiggyTech Suite</a>
        </div>
      </section>
      <MarketingFooter />
    </div>
