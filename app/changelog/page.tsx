...
const changelog = [
  {
    ...
    entries: [
      { type: 'feature', title: 'Automated Review Requests', description: 'Full feature set now available to all users.' },
  ...
  },
  {
    ...
    entries: [
      { type: 'launch', title: 'ZiggyReviews Launched', description: 'We built ZiggyReviews for our own business first. Now available for yours.' },
...
const typeColors: Record<string, string> = { feature: '#22c55e', improvement: '#f59e0b', fix: '#ef4444', launch: '#f59e0b' }
    ...
    <div className="bg-[#0a0a0a] min-h-screen" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
      ...
      <section className="pt-20 pb-16 px-4 text-center">
        <div className="max-w-3xl mx-auto">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#f59e0b] mb-4">Changelog</p>
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-4">What&apos;s new in ZiggyReviews</h1>
      ...
      <section className="py-12 px-4">
        <div className="max-w-3xl mx-auto space-y-12">
          {changelog.map((r) => (
            <div key={r.version} className={`bg-[#111111] border rounded-2xl overflow-hidden ${r.highlight ? 'border-[#f59e0b]/30' : 'border-[#1f1f1f]'}`}>
              <div className={`px-8 py-5 flex items-center justify-between ${r.highlight ? 'bg-[#f59e0b]/5' : 'bg-[#0d0d0d]'} border-b border-[#1f1f1f]`}>
                <div><p className="text-white font-bold text-lg">{r.version}</p><p className="text-[#b3b3b3] text-sm">{r.date}</p></div>
                {r.highlight && <span className="px-3 py-1 bg-[#f59e0b] text-white text-xs font-bold rounded-full">Latest</span>}
              </div>
